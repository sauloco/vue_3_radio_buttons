<template>
  <div>
    <div v-for="(option, index) in options" :key="index" class="rounded shadow-md my-5 p-5"
      ...
      <label :for="'option-' + index" class="flex items-center justify-between cursor-pointer">
        <div class="w-4 h-4 rounded-full border-2 border-red-300 flex items-center justify-center m-2">
      ...
      <div v-if="selectedOption === option.value" class="mt-2">
        <div class="border-t border-gray-200 pt-2">
          <a v-for="(detail, detailIndex) in option.details" :href="detail.target" target="_blank" rel="noopener noreferrer"
            class="w-full flex flex-row" :key="detailIndex">
            <span>{{ detail.text }}</span><span class="triangle-right"></span>
          </a>
        </div>
      </div>
    </div>
